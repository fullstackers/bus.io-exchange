var util = require('util')
  ...
  , debug = require('debug')('bus.io-exchange:exchange')
  , Queue = require('./queue')
...
function Exchange (queue, pubsub, handler) {
  ...
  /**
   * Used to attach to the Queue and PubSub instances
   * for capturing their errors
   *
   * @param {Error} err
   */
...
Exchange.prototype.queue = function (queue) {
  ...
  if (typeof queue === 'object' && queue instanceof Queue) {
    ...
    if (this._queue) {
      ...
      this._queue.removeListener('error', this.onError);
    }
    
    this._queue = queue;
    this._queue.on('message', this.onQueueMessage);
    this._queue.on('error', this.onError);
...
Exchange.prototype.pubsub = function (pubsub) {
  ...
  if (typeof pubsub === 'object' && pubsub instanceof PubSub) {
    ...
    if (this._pubsub) {
      ...
      this._pubsub.removeListener('error', this.onError);
    }
    
    this._pubsub = pubsub;
    this._pubsub.on('message', this.onPubSubMessage);
    this._pubsub.on('error', this.onError);
